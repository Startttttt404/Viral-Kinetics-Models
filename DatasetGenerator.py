...
def generate_viral_kinetics_dataset(variable_elements, num_choices, delta_t, t_0, t_n):
    ...
    for system in tqdm(systems, desc="Computing Solutions"):
        ...
        solution = model.solve(delta_t, t_0, t_n, Y_0)

        counter1 = 0
        counter2 = 1
        while counter2 < len(solution):
            x = solution[counter1]
            y = solution[counter2]
            counter1 += 1
            counter2 += 1
...
def generate_lstm_dataset(variable_elements, num_choices, delta_t, t_0, t_n):
    variable_elements = [element.lower() for element in variable_elements]
...
if __name__ == '__main__':
    generate_viral_kinetics_dataset([], 0, 0.01, 0, 6)
